...
class CommandInterpreter {
    ...
    private void insertStatement() {
        ...
        _input.next("values");
        int colums = table.columns();

        while (true) {
            _input.next("(");
            String[] inside = new String[colums];
            int k = 0;
            while (true) {
                inside[k] = literal();
                table.add(inside);
     ...
     *  or more Conditions. */
    private ArrayList<Condition> conditionClause(Table... tables) {
     ...
     *  token stream. */
    private Condition condition(Table... tables) {
        ...
        if (col2 == null) {
            condition = new Condition(col1, relation, val);
        } else {
            condition = new Condition(col1, relation, col2);
        }
